...
gamma <- rep(log(1), nrow(DAT))
#gamma <- rep(log(1.025), nrow(DAT))

# specify assumed interaction - assumed common for each trial here
# 1.3 for a 10-unit increase in age
# corresponds to a lnHR of 0.262364
# - for a 1-year increase in age, this corresponds to 0.0262364

interaction <- rep(log(1.3), nrow(DAT)) / 10
...
# generate the covariate Z of interest; I'll assume normal for now & that we 
...
z_cent <- matrix(ncol = nrow(DAT), nrow = obs)
for(i in 1:nrow(DAT)){
...
W    <- matrix(ncol = nrow(DAT), nrow = obs)
...
# Kalbfleisch book approach - prognostic effect of age = ln(1)
# interaction = .0262364
# ma_variance = .00007166
# ma_se = .00846503
# lower = .00964493
# upper = .04282787
# power = .87272892
...
# Kalbfleisch book approach - prognostic effect of age = ln(1.025)
#i nteraction = .0262364
# ma_variance = .00006981
# ma_se = .00835543
# lower = .00985975
# upper = .04261305
# power = .8810081
